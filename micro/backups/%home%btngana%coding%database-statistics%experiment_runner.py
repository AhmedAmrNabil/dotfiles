...
def restart_postgres():
    if os.name == "nt":
        ...
        os.system("sudo systemctl restart postgresql17")
...
def clear_cache():
    ...
    os.system(
        f'/opt/postgresql17/bin/psql -p {DB_PARAMS["port"]} -U {DB_PARAMS["user"]} {DB_PARAMS["dbname"]} -c "DISCARD ALL";'
    )
            ...
            for index_name, index_commands in indexes.items():
                ...
                for index, query in queries.items():
                    ...
                    for run in range(1, RUNS_PER_QUERY + 1):
                        ...
                        print(
                            f"Query {index}, Run {run}, Time: {execution_time:.2f} ms,indexConfig:{index_name},DbConfig:{config_name}"
                        )

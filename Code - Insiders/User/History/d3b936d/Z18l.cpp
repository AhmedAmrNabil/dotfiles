...
int main() {
    ...
    //     }
    // }
    counter = ending - floor((start_time - tb) / float(b));
    // tmp = (ta * tb + ceil(((start_time - tb) / float(b))) * b * ta + start_time * tb) / double(ta + tb);
    // cout << tmp << endl;
    // if (tmp >= (start_time + ta) || tmp == start_time) counter--;
